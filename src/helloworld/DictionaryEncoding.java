...
public class DictionaryEncoding
{
    ...
    public static void main(String[] args)
    {
        ...
        HashMap<String, Integer> encodeDict = new HashMap<>();
        for(String line: input.split("\n")){
            for (String word: line.split(" ")){
                if(!encodeDict.containsKey(word)){
                    encodeDict.put(word, encodeDict.size() + 1);
                }
            }
        }

        for (String word: encodeDict.keySet()){
            System.out.print("\n" + word + ": " + encodeDict.get(word));
        }
        System.out.println();
